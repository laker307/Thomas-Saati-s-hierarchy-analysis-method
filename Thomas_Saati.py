    ...
    if count > 0:
        for j in range(count):  # цикл
            for i in range(j,count):   # вложенный цикл
                ...
                b[j,i] = float(input())     # ввод данных попарного сравнения критериев справа от диагонали
                b[i,j] = np.around((1/(b[j,i])), 2)    # заполнение ячеек слева от диагонали
            sums.append(np.around(sum(b[j]),2)) # добавление суммы по строке в список
        print(b,'\nСуммы по каждой строке: \n',sums,'\n\n')
        f = sum(sums) # сумма всех сумм
        print('Итоговые коэффициенты: ',np.around(sums/f,2))  # вывод коэфф.
